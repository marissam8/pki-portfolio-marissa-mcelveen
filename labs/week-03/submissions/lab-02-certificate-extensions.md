...
2. This certificate is authorized to verify digital signatures during the TLS handshake agreement based on Key Usage.
3. The EKU field tells me that the certificate can authenticate a server.
4. This not a CA certificate. One way I can differentiate a CA from a leaf cert, is the validity period. A CA generally has a longer validity period of ten years or more for root, and intermediate with two-five. I can also tell the the difference between the two based on the length -leaf is longer in size.
